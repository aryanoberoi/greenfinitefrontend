...
const ChatBot = ({ sessionId, onClose }) => {
  ...
  return (
    <div className="w-full md:w-[48%] h-[33em] bg-white bg-opacity-90 rounded-xl shadow-2xl p-6 flex flex-col transition-all duration-300 hover:shadow-xl hover:scale-[1.005] font-sans relative z-0">
      ...
      <button
        onClick={onClose}
        className="absolute top-3 right-3 bg-transparent text-white rounded-full p-1 z-20"
        title="Minimize"
      >
        <span className="text-2xl font-bold text-gray-500 opacity-60 hover:opacity-90 transition-opacity">×</span>
      </button>
